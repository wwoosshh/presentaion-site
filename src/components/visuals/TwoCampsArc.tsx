  ...
  return (
    <svg aria-hidden viewBox="0 0 880 360" className="w-full max-w-[560px] my-2">
      ...
      <motion.path
        ...
        stroke="#e85d3c" strokeWidth="1.5" fill="none"
      ...
      <motion.path
        d={arcPath(cx, cy, r, 100, 180)}
        stroke="#5e5a52" strokeWidth="1.5" fill="none"
        initial={{ pathLength: 0 }} whileInView={{ pathLength: 1 }}
        viewport={{ once: false, amount: 0.4 }}
        transition={{ duration: 1.5, ease: [0.22, 1, 0.36, 1] }}
      />
      ...
      {Array.from({ length: LEFT_DOTS }).map((_, i) => {
        ...
        return (
          <motion.circle key={`l${i}`} cx={p.x} cy={p.y} r={3.5} fill="#e85d3c"
      ...
      {Array.from({ length: RIGHT_DOTS }).map((_, i) => {
        ...
        const p = polarToCartesian(cx, cy, r, t)
        return (
          <motion.circle key={`r${i}`} cx={p.x} cy={p.y} r={3} fill="#9d978a"
            initial={{ opacity: 0, scale: 0 }} whileInView={{ opacity: 1, scale: 1 }}
            viewport={{ once: false, amount: 0.4 }}
            transition={{ delay: 1.0 + i * 0.06, duration: 0.3 }}
          />
      ...
      >
        <motion.circle cx={cx} cy={cy - r} r={6} fill="#f0c674"
          animate={{ scale: [1, 1.2, 1], opacity: [1, 0.7, 1] }}
          transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
        />
        <text x={cx} y={cy - r - 14} textAnchor="middle"
          fontFamily="Gowun Batang" fontSize="14" fontWeight="700" fill="#f0c674">달 남극</text>
      </motion.g>
    </svg>
  )
}
